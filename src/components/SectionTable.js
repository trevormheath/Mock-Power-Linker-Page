...
function SectionTable({recordArray, treeArray, tableName}) {
    ...
    for(let i = 0; i < maxArraySize; i++) {
        ...
        let firstCol = recordArray[i] == null ? <DragDropEl tableName={tableName} content={<div></div>} id={"element" + i}/>
                                                 : <DragDropEl tableName={tableName} id={"element" + i} content={<AccordionElement person={recordArray[i]} key={recordArray[i].id}/>} />;
